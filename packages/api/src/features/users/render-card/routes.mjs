...
export const renderCardByUsernameRoute =
  (handler, format = 'svg') =>
...
/** @param {RenderCardHandler} handler @param {CardFormat} [format] */
export const renderCardByIdRoute =
  (handler, format = 'svg') =>
...
function parseAndValidateId(id) {
  ...
  if (Number.isNaN(numericId)) throw new Error('Invalid userID: ' + id)
  return numericId
}
...
function validateUsername(username) {
  if (typeof username !== 'string')
    throw new Error(
      'Invalid username: Expected string, but got ' + typeof username,
    )

  if (username.length === 0)
    throw new Error(
      'Invalid username: Username length must be greater than zero',
    )
}
